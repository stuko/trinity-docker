...
<div>
  <div class="content-module-heading cf" style="margin-bottom:20px;">
    <h3 class="fl">
<i class="fas fa-layer-group"></i>&nbsp;상품 포트폴리오 목록
    </h3>	
    <p>
      아래에 나와 있는 상품 포트플리오 정보를 상품 구매 룰 관리 화면에서 사용하시기 바랍니다.
    </p>
    <div class="row">
        <div class="col-sm-12">
           ...
           <!--<a href="/Anaconda.do?CMD=CMD_SEQ_104533676171581894345412&code=list" style="margin-right:10px;"><i class="fas fa-layer-group"></i>&nbsp; 상품 포트폴리오 관리</a>-->
            <input type="button" class="btn btn-secondary" name="Register" value="상품 포트폴리오 등록하기" onClick="javascript:goRegister();">
        </div>
     </div>
  </div>
  <div class="table-responsive">
    <table class="table">
      ...
      <tr>
        <th scope="col">번호</th>
        <th scope="col">상품 포트폴리오 이름</th>
        <th scope="col">상품 포트폴리오 가격</th>
        <th scope="col">상품 포트폴리오 할인율</th>
        <th scope="col">기능</th>
      </tr>
      </thead>
      <tbody>
...
%>
      <tr>
        <td><%=i+1%></td>
        <td><%=bc.getBusinessData(i).getFieldValue("PORT_NAME")%></td>
        <td><%=bc.getBusinessData(i).getFieldValue("PORT_FIXED_PRICE")%>원</td>
        <td><%=bc.getBusinessData(i).getFieldValue("PORT_FIXED_DC_RATE")%>%</td>
        <td>
        <span style="margin-right:5px"><button class="btn btn-warning" id="btnModify" onClick="javascript:fnModify('<%=bc.getBusinessData(i).getFieldValue("PORT_KEY")%>')">수정</button></span><span  style="margin-right:5px"><button class="btn btn-danger" id="btnDelete" onClick="javascript:fnDelete('<%=bc.getBusinessData(i).getFieldValue("PORT_KEY")%>')">삭제</button></span>
        </td>
      </tr>
<%
  }
...
%>
      <tr>
        <td colspan="3">상품 포트폴리오 정보가 없습니다.</td>
      </tr>
<%
}
%>
...
function fnPage(seq){
  ...
  document.location.href = "/Anaconda.do?CMD=CMD_SEQ_109386074031581898012096" + "&pos=" + seq;
}
...
function fnDelete(seq){
  ...
  var d = {PORT_KEY : seq};     
  $.ajax({
    url : '/Anaconda.json?CMD=CMD_SEQ_107953062181582008772586' ,
    type: 'POST',
    data : d,
    dataType : 'json',
...
function fnModify(seq){
   document.location.href = '/Anaconda.do?CMD=CMD_SEQ_108070811561582003672550&PORT_KEY=' + seq;
}
</script>
<%
...
<script>
function goRegister(){
  document.location.href="/Anaconda.do?CMD=CMD_SEQ_109231043021582001123009";
}
