...
<div>
  <div class="content-module-heading cf" style="margin-bottom:20px;">
    <h3 class="fl">
   <i class="fab fa-elementor"></i>&nbsp; "<%=bc.getBusinessData(0).getFieldValue("IT_NAME")%>" 의도 : 필수 질문 정보 입력
    </h3>	
  <span style="text-align:right">
   <button class="btn btn-primary" id="backToList"><< 의도 목록으로 돌아가기</button>
  </span>
<% if(!"0".equals(bc.getBusinessData(0).getFieldValue("CNT"))){ %>
  <span style="text-align:right">
  ...
  <div class="table-responsive">
    <table class="table">
      ...
      <tr>
        ...
        <tr><td>필수 질문 순서</td><td><input type="text" name="SLOT_SEQ" id="SLOT_SEQ" class="styled" size="10">&nbsp;번째</td></tr>
        <tr><td>필수 질문 항목 명</td><td><input type="text" name="SLOT_NAME" id="SLOT_NAME" class="styled" size="70"></td></tr>
      <tbody>
      <tr>
      </tr>
      </tbody>
...
function save(){
  with(frmSlot){
    ...
    var d = {SLOT_SEQ : $('#SLOT_SEQ').val(),SLOT_NAME : $('#SLOT_NAME').val(),IT_KEY: $('#IT_KEY').val()};

    $.ajax({
      url: "/Anaconda.json?CMD=CMD_SEQ_107221174531582509018188",
      data: d,
      type : "POST",
      dataType: "json",
