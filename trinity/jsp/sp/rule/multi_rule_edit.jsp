...
<div id="div_rule_menu" style="width:400px;position:absolute;display:none;background-color:white;border: 2px double #CCCCCC; z-index:200;">
  <form name="frmMenu">
    <ul style="list-style:none ;padding: 10px 25px 10px 25px;">
      <p style="height:30px">
        <img src="/jsp/bestmember/img/important.png" width="20px" height="20px" align="absmiddle"></img>&nbsp;룰 정보 관리&nbsp;<a style="text-decoration:none;" href='javascript:fnShowOrHide("div_rule_menu");'><img src='/jsp/anaconda/img/close.png' width='20px' height='20px' border='0' align='absmiddle' />&nbsp;닫기</a>
      </p>
      <li><a style="text-decoration:none;" href='javascript:fnNull();' onClick='javascript:goDetail();'><img src="/jsp/bestmember/img/shortcut.png" border="0" align="absmiddle"></img>&nbsp;Shortcuts</a></li>    
      <li><a style="text-decoration:none;" href='javascript:fnNull();' onClick='javascript:goShowRule();'><img src="/jsp/bestmember/img/shortcut.png" border="0" align="absmiddle"></img>&nbsp;룰 자세히보기</a></li>
  ...
  <form name="frmRule" method="post" action="/Anaconda.do">
    ...
    <table class="table table-condensed">
      ...
      <tbody>
        ...
        <tr>
          <td>룰 실행 스크립트<button type="button" id="btn_help" class="btn btn-primary" onClick="javascript:showFunctionList();">Help</button></td>
          <td><textarea class="form-control" rows="10" name="rule"></textarea></td>
        </tr>
        <tr>
          <td>구분자(기본값: ,)</td>
  ...
  function getMoveTargetList(){
      ...
      post(d,'CMD_SEQ_107563812781614813114572',(res)=>{
          ...
          <tr id='answers'>
            ...
            (1) &nbsp;<input type="text" name="Answer1" class="styled"  style="height:23px;margin:5px;" size="32" ></input><br/>
            <input type="hidden" name="QCCode2">
            (2) &nbsp;<input type="text" name="Answer2" class="styled"  style="height:23px;margin:5px;" size="32" ></input><br/>
            <input type="hidden" name="QCCode3">
            (3) &nbsp;<input type="text" name="Answer3" class="styled"  style="height:23px;margin:5px;" size="32"></input><br/>
            <input type="hidden" name="QCCode4">
            (4) &nbsp;<input type="text" name="Answer4" class="styled"  style="height:23px;margin:5px;" size="32"></input><br/>
            <input type="hidden" name="QCCode5">
            (5) &nbsp;<input type="text" name="Answer5" class="styled"  style="height:23px;margin:5px;" size="32"></input><br/>
            <input type="hidden" name="QCCode6">
            (6) &nbsp;<input type="text" name="Answer6" class="styled"  style="height:23px;margin:5px;" size="32"></input><br/>
            <input type="hidden" name="QCCode7">
            (7) &nbsp;<input type="text" name="Answer7" class="styled"  style="height:23px;margin:5px;" size="32"></input>
            </td>
          </tr>
          </table>
          </td>
  ...
  if(object.length != null && object.length != "")  {
    ...
    for(var i=0 ; i < count ; i++){
      ...
      if(object.response[i].parent_seq == -1) {
        ...
        a.innerHTML =   "<br/><img src='/jsp/bestmember/img/home.png' width='20'>"   +  "&nbsp;<b>" + object.response[i].context_name + '<br>' + (object.response[i].rule != null && object.response[i].rule != 'null' ? object.response[i].rule : '') + "</b>&nbsp;&nbsp;"  ;
      }else{
         ...
         if(object.response[i].rule_condition == 'true' || object.response[i].rule_condition == '0') {
           ...
           a.innerHTML = "<br/><img src='/jsp/sp/img/true.png' width='15'>"  + "&nbsp;<b>" + object.response[i].rule+ '</b><p>' + object.response[i].msg + "&nbsp;&nbsp;</p>"  ;
         } else{
           div2.className = "div_keyword_false";
           a.innerHTML =  "<br/><img src='/jsp/sp/img/false.png' width='15'>"   +  "&nbsp;<b>"  + object.response[i].rule+ '</b><p>' + object.response[i].msg + "&nbsp;&nbsp;</p>"  ;
         }
      }
      ...
      span_1.innerHTML = "Rule ID : " + object.response[i].seq + " (Child Rule ID : <a href=\"javascript:fnNull();\" onClick=\"javascript:goNextPage('"+childRuleId +"','" +object.response[i].context_name+"');\">"+
                         "<b style='color:blue' title='Go to a child rule'>" + object.response[i].context_name +"["+priority+"]</b>["
                        +childRuleId+"]</a>)" + delButton;
...
function fnRemoveSubRule(arg1){
    if(confirm("Do you want to delete the sub-rule (reference rule)?")){
...
<div style="z-index:10">
  ...
  <h3 ><img src="/jsp/sp/img/realtime_message.png" width="40px">&nbsp;대화 시나리오 관리</h3>
     <!-- Rule Tree -->
            <div>
...
<div>
<pre>
  (1) 저장('XXXX') 현재 입력한 메시지를 XXXX 이름으로 저장 해 줌.
  (2) equal('XXXX') 현재 입력한 메시지가 XXXX 와 같으면 True.
  (3) like('XXXX') 현재 입력한 메시지가 XXXX 와 비슷하면 True.
  (4) intentLike('XXXX',10) 현재 입력한 메시지가  XXXX의도일 확률이 10%이면 True. 
  (5) have('XXXX,XX,XXXXX') 현재 입력한 메시지에 , 로 분리된 단어 들이 포함되면 True. 
</pre>
</div>
</div>
              <div>           
