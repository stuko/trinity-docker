...
function createDB(){
  ...
  post(d,"CMD_SEQ_108475078891560743573347",()=>{
     showPopup("성공","축하합니다, 챗봇 데이터베이스가 생성을 요청하였습니다. (새 데이터베이스 적용은 최다 1시간정도 소요됩니다.)",null);
     confirmPopup('성공','새로운 챗봇 데이터 베이스가 적용되기 위해서는 최대 1시간 정도 소요됩니다., 다시 로그인을 하신후, 사용하시기 바랍니다.', ()=>{
...
<div class="container-fluid" style="z-index:10;padding-top:20px;">
     <div class="card mb-3" style="padding-bottom:40px;">
           <div class="card-header">
             <h3><img src="/jsp/sp/img/login.png" width="30px">&nbsp; 정보 수정</h3>
           </div>
          * DUNA 챗봇 서비스를 이용해 주셔서 감사합니다.
      </div>
     ...
     <div class="card mb-3" style="padding-bottom:40px;">
           <div class="card-header">
             ...
             <%
              if(needChatBotDB){
             %>
             * 아직 챗봇을 생성하지 않았습니다. 챗봇이 필요하시면 생성 버튼을 이용해 주세요 &nbsp;
                 <button type="button" id="btn_chatbotdb" class="btn btn-danger btn-lg" onclick="javascript:createDB();">Create
 ChatBot</button>
             <%}else{%>
             <div>
              <ul>
             <li> 챗봇이 생성되었습니다. <i class="fas fa-certificate fa-2x" style="color:#FF5733;"></i></li>
             <li> 현재 회원님의 챗봇은 <%=total%>개의 DB 연결을 사용중이십니다.(시스템의 안정성을 위해 기본적으로 100개 이하가 할당됩니다)</li>
              </ul>
             </div>
...
<pre>
  (1) Javascript 소스 포함 
    <%=domain%>/Anaconda.do?CMD=CMD_SEQ_100697583901583557897409&clientId=CB_20191222115558172-876659545&secretKey=SK_20191222115558173-750635315
  (2) CSS 소스 포함
    <%=domain%>/Anaconda.do?CMD=CMD_SEQ_107379266481583583370476
</pre>
...
<form id="frmModify" name="frmModify" method="POST" action="">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="domain" value='<%= (bc2 == null || bc2.size() == 0) ? "" : bc2.getBusinessData(0).getFieldValue("domain")%>' name="domain">(* 챗봇을 사용하시려는 사이트의 도메인 ex: shop.cafe24.com )
  </div>
  <div class="form-group">
