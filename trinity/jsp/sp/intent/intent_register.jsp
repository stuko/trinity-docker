...
<% if(!"0".equals(request.getParameter("IT_COUNT"))){ %>
  <span style="text-align:right">
   <button class="btn btn-primary" id="backToIntentList");"><< 대화 의도 목록으로 돌아가기</button>
  </span>
<% } %>
  <span style="text-align:right">
   <button class="btn btn-secondary" id="save"">추가하기</button>
  </span>

  </div>
  <form name="frmIntent" id="frmIntent" method="POST">
  <input type="hidden" name="IG_KEY" id="IG_KEY" value="<%=request.getParameter("IG_KEY")%>" />
  <div class="table-responsive">
    <table class="table">
      ...
      <tr><td>의도</td><td><input type="text" name="IT_NAME" id="IT_NAME" class="styled" size="70"></td></tr>
      <tr><td>설명</td><td><input type="text" name="IT_DESC" id="IT_DESC" class="styled" size="70"></td></tr>
      <tbody>
      <tr>
      </tr>
      </tbody>
...
$('#backToIntentList').on('click',function(event){
  document.location.href = '/Anaconda.do?CMD=CMD_SEQ_106158749731580254604919&IG_KEY=<%=request.getParameter("IG_KEY")%>';
});
...
function save(){
  with(frmIntent){
    ...
    var d = {IT_NAME : $('#IT_NAME').val(), IT_DESC : $('#IT_DESC').val(), IG_KEY : $('#IG_KEY').val()};

    $.ajax({
      url: "/Anaconda.json?CMD=CMD_SEQ_106200478241580277964463",
      data: d,
      type : "POST",
      dataType: "json",
