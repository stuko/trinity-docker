...
function fnSelect()
{
	with(frmSelectList)
	{
		for(var i = 0; i < wac_check_list.length ; i++)
		{
           ...
           {
				var hidden = createElement("input","view_list","checkbox");
				hidden.value = wac_check_list[i].value;
				appendChild(hidden);
		   }
		}
		callAjaxByForm("searchView", "frmSelectList");
		//submit();
	}
}
</script>
...
<input type="hidden" name="type" value="searchView"></input>
		...
		<%
			try
			{
				...
				for (int ii = 0 ;it.hasNext() && ii <= MAX_COL+1 ; ii++)
				{
					...
					out.println("<tr>");
					out.println("<td align=\"left\" > "+ viewCollection.getVIEWCOLLECTION_KOR_NAME() +"</td>");
					out.println("<td align=\"center\" > <input type='checkbox' name='wac_check_list' value='" + viewCollection.getVIEWCOLLECTION_NAME() + "'></input></td>");
					out.println("</tr>");
				}
			} catch (Exception e) {
				e.printStackTrace();
