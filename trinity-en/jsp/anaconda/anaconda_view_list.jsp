...
function fnViewCreate()
{
	...
	//window.open("/Anaconda.do?CMD=CMD_SEQ_105760595001287408553144","","height=450,width=800,left=200,top=20,status=no, location=no, toolbar=no,resizable=yes,scrollbars=yes");
}
...
<form name="searchFrm" method="POST" action="/Anaconda.do?CMD=CMD_SEQ_100600677051287893203559">
		...
		<div class="post"><div class="title-right"><h2><font><b>화면 목록</b></font></h2></div></div>
			<span class="fr"><input type="button" class="button white" name="" value="화면 그룹 (Group) 생성" colspan="2" onClick="javascript:fnViewCreate();"></span><span class="fr">
				</span><span class="fr"><input type="button" class="button white"  name="" value="UI 요소 목록" colspan="2" onClick="javascript:fnUIList();"></span><span class="fr"><input type="button" class="button white"  name="" value="레이아웃 (Layout) 목록화면" colspan="2" onClick="javascript:fnLayoutList();"></span>
<span class="fr"><input type="button"  id="actionGroupShow" class="button white" name="Register" value="전체보기" onClick="javascript:fnShowOrHideActionGroup('actionGroupShow','divContext');fnNull();"></span>

<%if (strSessionValue.equals(InitClass.getInstance().getData(InitClass.SA_ID))) {%><span class="fr"><input type="button" class="button white" value="현재 사용자로 일괄변경" onClick="javascript:fnAllChangeUser();"></input></span><%} %>
...
%>
		<td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img src="/jsp/anaconda/img/box.png" width="13px" height="13px" border=0 align="absmiddle">&nbsp;&nbsp;<a href="javascript:fnSearchWACList('<%=strWACName %>');"><%=strWACKName %></a>&nbsp;&nbsp;
		</td>
<%
			if(iTr == 0)
			{
...
<td>
		...
		<%
			try
			{
				...
				if( request.getParameter("srh_param") != null
					&& !request.getParameter("srh_param").trim().equals("")
				...
				{
					// out.println("<tr align=\"center\"><td class=\"list_top\" colspan=\"11\">전체 리스트</td></tr>");
				...
				else
				{
				   ...
				   //out.println("<tr align=\"center\"><td  class=\"list_top\"  colspan=\"5\">조회 결과("+ searchGubun +")</td></tr>");
				}
          ...
          %>
 		  <%
			...
			for (;it.hasNext();)
				{    				
					viewCollection = (ViewCollection)it.next();
					// out.println(viewCollection.getVIEWCOLLECTION_NAME() + "/" +viewCollection.size() );
			    if(strWAC.equals("ALL") || strWAC.equals(viewCollection.getVIEWCOLLECTION_NAME()))
			    {
				    ...
				    for(int j =0 ;iterator.hasNext();j++)
				    {
						...
						//out.println("<td  height=\"20\"  bgcolor=\"#FFFFFF\" style=\"padding:3 5 2 7;\">"+view.getLAYOUT()+"</td>");
						//out.println("<td  height=\"20\"  bgcolor=\"#FFFFFF\" style=\"padding:3 5 2 7;\">"+view.getBODY()+"</td>");
						if(i == 0)
						{
