...
<div>
  <div class="content-module-heading cf" style="margin-bottom:20px;">
    <h3 class="fl">
<i class="fas fa-hands-helping"></i>&nbsp;상품 서비스 목록
    </h3>	
    <p>
      아래에 나와 있는 서비스 정보를 상품 구매 룰 관리 화면에서 사용하시기 바랍니다.
    </p>
    <div class="row">
        <div class="col-sm-12">
           ...
           <a href="/Anaconda.do?CMD=CMD_SEQ_104533676171581894345412&code=list" style="margin-right:10px;"><i class="fas fa-layer-group"></i>&nbsp; 상품 포트폴리오 관리</a>
            <input type="button" class="btn btn-secondary" name="Register" value="서비스 등록하기" onClick="javascript:goRegister();">
        </div>
     </div>
  </div>
  <div class="table-responsive">
    <table class="table">
      ...
      <tr>
        ...
        <th scope="col">서비스 등록일</th>
        <th scope="col">서비스 가능 여부</th>
        <th scope="col">기능</th>
      </tr>
      </thead>
      <tbody>
...
%>
      <tr>
        ...
        <td><%=bc.getBusinessData(i).getFieldValue("SVC_REGDATE")%></td>
        <td><%=("Y".equals(bc.getBusinessData(i).getFieldValue("SVC_YN")) ? "사용 가능" : "사용 불가능")%></td>
        <td>
        <span style="margin-right:5px"><button class="btn btn-warning" id="btnModify" onClick="javascript:fnModify('<%=bc.getBusinessData(i).getFieldValue("SVC_KEY")%>')">수정</button></span><span  style="margin-right:5px"><button class="btn btn-danger" id="btnDelete" onClick="javascript:fnDelete('<%=bc.getBusinessData(i).getFieldValue("SVC_KEY")%>')">삭제</button></span>
        </td>
      </tr>
<%
  }
...
function fnPage(seq){
  ...
  document.location.href = "/Anaconda.do?CMD=CMD_SEQ_103736959391582064326654" + "&pos=" + seq;
}
...
function fnDelete(seq){
  ...
  var d = {SVC_KEY: seq};     
  $.ajax({
    url : '/Anaconda.json?CMD=CMD_SEQ_100841360101582071107045' ,
    type: 'POST',
    data : d,
    dataType : 'json',
...
function fnModify(seq){
   document.location.href = '/Anaconda.do?CMD=CMD_SEQ_102284620851582067595610&SVC_KEY=' + seq;
}
</script>
<%
...
<script>
function goRegister(){
  document.location.href="/Anaconda.do?CMD=CMD_SEQ_100180697941582066593646";
}
