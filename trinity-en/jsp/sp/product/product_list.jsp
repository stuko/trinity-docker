...
<%
BusinessCollection bc = RequestHelper.getBusinessCollection(request,"PRODUCT_list");
%>
<div>
  <div class="content-module-heading cf" style="margin-bottom:20px;">
    <h3 class="fl">
<i class="fas fa-images"></i>&nbsp;상품 목록
    </h3>	
    <p>
      아래에 나와 있는 상품 정보를 상품 구매 룰 관리 화면에서 사용하시기 바랍니다.
    </p>
    <div class="row">
        <div class="col-sm-12">
           ...
           <a href="/Anaconda.do?CMD=CMD_SEQ_104533676171581894345412&code=list" style="margin-right:10px;"><i class="fas fa-layer-group"></i>&nbsp; 상품 포트폴리오 관리</a>
            <input type="button" class="btn btn-secondary" name="Register" value="상품 등록하기" onClick="javascript:goRegister();">
        </div>
     </div>
  </div>
...
%>
      <tr>
        <td><%=i+1%></td>
        <td><%=bc.getBusinessData(i).getFieldValue("PRD_NAME")%></td>
        <td><%=bc.getBusinessData(i).getFieldValue("PRD_FIXED_PRICE")%>원</td>
        <td><%=bc.getBusinessData(i).getFieldValue("PRD_FIXED_DC_RATE")%>%</td>
        <td>
        <span style="margin-right:5px"><button class="btn btn-warning" id="btnModify" onClick="javascript:fnModify('<%=bc.getBusinessData(i).getFieldValue("PRD_KEY")%>')">수정</button></span><span  style="margin-right:5px"><button class="btn btn-danger" id="btnDelete" onClick="javascript:fnDelete('<%=bc.getBusinessData(i).getFieldValue("PRD_KEY")%>')">삭제</button></span>
        </td>
      </tr>
<%
  }
...
<script>
function goRegister(){
  document.location.href="/Anaconda.do?CMD=CMD_SEQ_109158715331581941330209";
}
