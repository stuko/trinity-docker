...
<%
BusinessCollection bc = RequestHelper.getBusinessCollection(request,"sql");
%>
...
<table class="table_hydra">
  ...
  <tr>
    ...
    <td>
	<div name="SQL_RESULT" id="SQL_RESULT" style="height:200px;overflow-y:auto;">
          <ul>
<%

if(bc != null && bc.size() > 0){
   for(int i = 0; i < bc.size() ; i++){
      out.println("<li><a href='javascript:fnSet(\""+BusinessHelper.convert(bc.getBusinessData(i).getFieldValue("DATA_SOURCE"))+"\",\""+BusinessHelper.convert(bc.getBusinessData(i).getFieldValue("SQL"))+"\")'>"+bc.getBusinessData(i).getFieldValue("SQL")+"</a>");
   }
}
%>
         </ul>
...
function addSql(){
   var cur_sql = $('#SQL').val();
   var cur_result = $('#SQL_RESULT').html();
   $('#SQL_RESULT').html('<li>'+ cur_sql + '\n' + cur_result);
}

function fnSet(data_source, sql){
  $('#DATASOURCE').val(data_source.replaceAll('<BR>','\n'));
  $('#SQL').val(sql.replaceAll('<BR>','\n'));
}
